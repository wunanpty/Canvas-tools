...
def create_user(account_id, user_name, short_name, sortable_name, time_zone, locale, birthdate, unique_id, password, sis_user_id, email_address):
    ...
    print('##### # POST /api/v1/accounts/:account_id/users #####')
    time_diff_create_each = (b - a).total_seconds() * 1000
    print("create_user", time_diff_create_each)
    time_diff_create.append(time_diff_create_each)
...
def enroll_user_with_sis_id(course_id, sis_id, role, section_id):
    ...
    r = requests.post(url, headers = header, data=payload)
    b = datetime.datetime.now()
    time_diff_enroll_each = (b - a).total_seconds() * 1000
    time_diff_enroll.append(time_diff_enroll_each)
    print('##### POST /api/v1/courses/:course_id/enrollments #####')
    print("enroll_user", time_diff_enroll_each)
    ...
    if options.users:
        for user in test_users:
            if not existing_user_by_sis_id(all_users, user['sis_user_id']):
                print('############################ if not existing_user_by_sis_id NANWU##########################')
                # create the user
        ...
        time_diff_create_mean = sum(time_diff_create) / len(time_diff_create)
        print("time_diff_create_mean", time_diff_create_mean)
        #write to a csv file
        filename1 = "baremetal_create_baseline_1.csv"
        with open(filename1, 'w') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow(time_diff_create)
            csvwriter.writerow(["time_diff_create_mean",time_diff_create_mean])
        ...
        for user in test_users:
            #print('##### if not existing_user_in_course_by_sis_id #####')
        ...
        time_diff_enroll_mean = sum(time_diff_enroll) / len(time_diff_enroll)
        print("time_diff_enroll_mean", time_diff_enroll_mean)
        #write to a csv file
        filename2 = "baremetall_enroll_baseline_1.csv"
        with open(filename2, 'w') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow(time_diff_enroll)
            csvwriter.writerow(["time_diff_enroll_mean",time_diff_enroll_mean])
