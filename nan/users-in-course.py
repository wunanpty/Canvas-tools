...
def users_in_course(course_id):
    ...
    if r.status_code == requests.codes.ok:
        ...
        # count the number of paginated response for API: GET /api/v1/courses/:course_id/enrollments
        global paginated_count
        paginated_count = 0
        
        while r.links.get('next', False):
            #print('### in paginated NANWU#####')
...
def main():
    ...
    if (len(remainder) < 1):
        ...
        for x in range(iteration):
            ...
            time_diff_each = (b - a).total_seconds() * 1000
            #print('############################NANWU##########################\n')
            time_diff.append(time_diff_each)
        
        time_diff_mean = sum(time_diff) / len(time_diff)
        #write to a csv file
        filename1 = "BareMetal_users_50_baseline_0506.csv"
        with open(filename1, 'w') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow(time_diff)
            csvwriter.writerow(["time_diff_mean",time_diff_mean])
            csvwriter.writerow(["paginated_count", paginated_count])
            
        #print("time_diff", time_diff)
        time_diff_mean = sum(time_diff) / len(time_diff)
        print("users in course " + course_id + " total time spent", time_diff_mean)
        ...
        print("list courses total time spent", time_diff_mean_1)
            ...
            #print('############################NANWU##########################\n')
            #print("list courses total time spent", (d - c).total_seconds() * 1000)
            #print('############################NANWU##########################\n')
            
            #print(time_diff_each)
            #print('############################NANWU##########################\n')
            #print("users in course " + course_id + " total time spent", (b - a).total_seconds() * 1000)
            #print('############################NANWU##########################\n')
